...
class LandingPageResource extends Resource
{
    ...
    {
        return $form->schema([
            Section::make(__('section.landing_page_details'))->columns(['default' => 1, 'md' => 2])->schema([
                Select::make('marketing_campaign_id')
                            ...
                            ->mapWithKeys(function (MarketingCampaign $campaign): array {
                                ...
                                return [
                                    $campaign->id => $campaign->name
                                        ...
                                        .($services !== ''
                                            ? $services
                                            : __('common.service_not_configured')),
                                ];
                            })
                            ->all()
                    )
                    ...
                    ->helperText(__('helper.marketing_campaign_display_scope')),
                ...
                TextInput::make('slug')
                    ->label(__('uiux.form.public_slug'))
                    ->required()
                    ->maxLength(255)
                    ->unique(ignoreRecord: true)
                    ->helperText(__('uiux.form.slug_public_helper')),
            ...
            Section::make(__('field.landing_page_service'))
                ->description(__('helper.landing_page_form_reuse'))
                ->columns(['default' => 1, 'md' => 2])
                ->schema([
                    Select::make('service_id')
                        ->label(__('field.service_primary'))
                        ->options(
                        ...
                        ->helperText(
                            fn (Get $get): string => filled($get('marketing_campaign_id'))
                                ? __('helper.landing_page_service_campaign')
                                : __('helper.landing_page_service_context')
                        ),
                    Select::make('service_package_ids')
                        ->label(__('field.allowed_packages'))
                        ->multiple()
                        ...
                        ->helperText(__('helper.landing_page_packages')),
                ]),
            Section::make(__('section.landing_page_forms'))
                ->description(__('helper.landing_page_forms'))
                ->schema([
                            ...
                            Tabs\Tab::make(
                                ...
                                ->schema([
                                    Select::make(
                                        ...
                                        ->label(__('field.personal_form_template'))
                                        ->options(
                                        ...
                                        ->searchable()
                                        ->preload()
                                        ->helperText(__('helper.landing_page_personal_form')),
                                ]),
                            ...
                            Tabs\Tab::make(
                                ...
                                ->schema([
                                    Select::make(
                                        ...
                                        ->label(__('field.business_form_template'))
                                        ->options(
                                            fn (): array => FormTemplate::query()
                                                ->where(
                                                    'audience_type',
                                        ...
                                        ->searchable()
                                        ->preload()
                                        ->helperText(__('helper.landing_page_business_form')),
                                ]),
                        ])
                        ->columnSpanFull(),
                ]),
            Section::make(__('field.form_tone'))
                ->description(__('helper.theme_detection'))
                ->columns(['default' => 1, 'md' => 2, 'xl' => 3])
                ->schema([
                    ColorPicker::make('theme_tokens.primary')
                        ->label(__('field.primary_color'))
                        ->default('#2563eb'),

                    ColorPicker::make('theme_tokens.primary_hover')
                        ->label(__('field.hover_color'))
                        ->default('#1d4ed8'),

                    ColorPicker::make('theme_tokens.background')
                        ->label(__('field.form_section_background'))
                        ->default('#f8fafc'),

                    ColorPicker::make('theme_tokens.surface')
                        ->label(__('field.form_background'))
                        ->default('#ffffff'),

                    ColorPicker::make('theme_tokens.text')
                        ->label(__('field.text_color'))
                        ->default('#0f172a'),

                    ColorPicker::make('theme_tokens.muted_text')
                        ->label(__('field.muted_text_color'))
                        ->default('#64748b'),

                    ColorPicker::make('theme_tokens.border')
                        ->label(__('field.border_color'))
                        ->default('#cbd5e1'),

                    ColorPicker::make('theme_tokens.danger')
                        ->label(__('field.error_color'))
                        ->default('#dc2626'),

                    TextInput::make('theme_tokens.radius')
                        ->label(__('field.border_radius'))
                        ->default('12px')
                        ->placeholder('12px'),
                ]),
            Section::make(__('section.automation'))->columns(1)->schema([
                TagsInput::make('auto_tag_names')->label(__('field.auto_tag_names')),
                TagsInput::make('auto_list_names')->label(__('field.auto_list_names')),
                Toggle::make('auto_create_tags')->label(__('field.auto_create_tags'))->default(false),
                Toggle::make('auto_create_lists')->label(__('field.auto_create_lists'))->default(false),
    ...
    public static function table(Table $table): Table
    {
        return $table->columns([
            ...
            TextColumn::make('status')
                ...
                ->color(fn ($state): string => $state instanceof LandingPageStatus
                    ? $state->color()
                    : (LandingPageStatus::tryFrom((string) $state)?->color() ?? 'gray')),
            ...
            TextColumn::make('service.name')->label(__('field.service_primary'))->toggleable(),
            TextColumn::make('published_at')->label(__('field.published_at'))->dateTime('d/m/Y H:i')->sortable(),
                    ...
                    Action::make('publish')
                        ...
                        ->action(function (LandingPage $record): void {
                            ...
                            if ($record->marketing_campaign_id !== null) {
                                try {
                                    ...
                                    $message = collect($exception->errors())
                                        ->flatten()
                                        ->first() ?? __('validation.landing_campaign_service_mismatch');

                                    Notification::make()
                                        ->title(__('notification.landing_page_publish_failed'))
                                        ->body($message)
                                        ->danger()
                                        ->send();
                            ...
                            if ($usesServiceInterest && $record->service_id === null) {
                                Notification::make()
                                    ->title(__('notification.landing_page_publish_failed'))
                                    ->body(__('validation.landing_service_required'))
                                    ->danger()
                                    ->send();
                            ...
                            if (! $hasPersonalForm || ! $hasBusinessForm) {
                                ...
                                if (! $hasPersonalForm) {
                                    $missing[] = __('field.form_personal');
                                }

                                if (! $hasBusinessForm) {
                                    $missing[] = __('field.form_business');
                                }

                                Notification::make()
                                    ->title(__('notification.landing_page_publish_failed'))
                                    ->body(
                                        __('notification.landing_missing_prefix', ['items' => implode(', ', $missing)])
                                    )
                                    ->danger()
                                    ->send();
