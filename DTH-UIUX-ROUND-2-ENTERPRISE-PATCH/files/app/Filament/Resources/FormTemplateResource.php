                    ...
                    Repeater::make('fields')
                        ...
                        ->itemLabel(
                            function (array $state): string {
                                ...
                                return __('field.form_field');
                            }
                        )
                        ->schema([
                            TextInput::make('label')
                                ->label(__('field.field_label'))
                                ->required()
                                ->maxLength(255)
                                ->live(onBlur: true)
                                ->afterStateUpdated(function (?string $state, Set $set, Get $get): void {
                                    if (blank($get('field_key'))) {
                                        $set('field_key', Str::snake(Str::ascii((string) $state)));
                                    }
                                }),
                            Hidden::make('field_key')
                                ->default(fn (): string => 'field_'.Str::lower(Str::random(8))),
                            ...
                            KeyValue::make('options')
                                ...
                                ->keyLabel(__('field.technical_value'))
                                ->valueLabel(__('field.display_label'))
                                ->addActionLabel(__('action.add_option'))
                                ->helperText(__('helper.form_field_option_scope'))
                            ...
                            Select::make('contact_mapping')
                                ...
                                ->helperText(__('helper.form_mapping_scope'))
                            ...
                            TextInput::make('sort_order')
                                ...
                                ->numeric()
                                ->default(0)
                                ->helperText(__('uiux.form.sort_order_helper')),
                        ])
                        ->columns(['default' => 1, 'md' => 2])
                        ->columnSpanFull(),
                ]),
        ...
        return $table->columns([
            ...
            TextColumn::make('audience_type')
                ...
                ->color(function ($state): string {
                    ...
                    return \App\Support\Ui\BadgePalette::audience($value);
                }),
            TextColumn::make('status')
                ...
                ->color(fn ($state): string => $state instanceof FormTemplateStatus
                    ? $state->color()
                    : (FormTemplateStatus::tryFrom((string) $state)?->color() ?? 'gray')),
