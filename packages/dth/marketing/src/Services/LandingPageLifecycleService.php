...
final class LandingPageLifecycleService
{
    ...
    public function assertPublishReady(LandingPage $page): void
    {
        ...
        $usesServiceInterest = $personal->fields()
            ->where(function ($query): void {
                $query->where('semantic_role', SemanticFieldRole::ServiceInterest->value)
                    ->orWhere('contact_mapping', 'lead.service_interest');
            })
        ...
        if ($usesServiceInterest && blank($page->service_reference)) {
            throw ValidationException::withMessages([
                'service_reference' => 'A service is required because an attached form contains a service/package interest field.',
            ]);
        }
    ...
    public function transition(LandingPage $page, LandingPageStatus $target): LandingPage
    {
        ...
        $page->save();
        $action = match ($target) {
            LandingPageStatus::Published => 'landing_page.published',
            LandingPageStatus::Draft => 'landing_page.unpublished',
            LandingPageStatus::Archived => 'landing_page.archived',
        };
        $this->audit->log($action, $page, ['status' => $from], ['status' => $target->value]);
