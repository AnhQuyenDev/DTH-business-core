...
final class LandingPageThemeService
{
    ...
    public function detectFromHtml(string $html): array
    {
        ...
        $tokens['background'] = $this->normalizeHex(
            ...
            ?? $variables['bg']
            ?? null
        ) ?? $this->detectBodyColor($html, 'background(?:-color)?') ?? $tokens['background'];
        ...
        $tokens['text'] = $this->normalizeHex(
            ...
            ?? $variables['text']
            ?? null
        ) ?? $this->detectBodyColor($html, 'color') ?? $tokens['text'];

        if (preg_match('/border-radius\s*:\s*(\d+(?:\.\d+)?(?:px|rem))/i', $html, $match)) {
    ...
    /** @param array<string, mixed>|null $tokens @return array<string, string> */
    public function normalize(?array $tokens): array
    {
        $normalized = $this->defaults();

        foreach ($tokens ?? [] as $key => $value) {
            if (array_key_exists((string) $key, $normalized) && filled($value)) {
    ...
    private function extractCssVariables(string $html): array
    {
        ...
        preg_match_all('/--([a-z0-9-_]+)\s*:\s*([^;}]+)/i', $html, $matches, PREG_SET_ORDER);
    ...
    private function detectPrimaryColor(string $html): ?string
    {
        ...
        $ignored = [
            ...
            '#cbd5e1', '#64748b', '#475569', '#1e293b', '#0f172a',
        ];
    ...
    private function detectBodyColor(string $html, string $propertyPattern): ?string
    {
        if (! preg_match('/body\s*\{[^}]*'.$propertyPattern.'\s*:\s*(#[0-9a-f]{3,6})/i', $html, $match)) {
            return null;
        }
