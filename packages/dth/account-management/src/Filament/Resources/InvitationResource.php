...
class InvitationResource extends Resource
{
 ...
 public static function form(Schema $schema):Schema{return $schema->components([Section::make(UiText::get('sections.invitation','Thông tin lời mời'))->icon('heroicon-o-envelope')->schema([
  TextInput::make('name')->label(UiText::get('common.fields.name','Họ và tên'))->maxLength(255)->helperText(UiText::get('fields.invitation_name_help','Tên người được mời, dùng để cá nhân hóa email mời (không bắt buộc).')),TextInput::make('email')->label(UiText::get('common.fields.email','Email'))->email()->required()->maxLength(255)->helperText(UiText::get('fields.invitation_email_help','Email nhận lời mời và liên kết kích hoạt tài khoản.')),DateTimePicker::make('expires_at')->label(UiText::get('fields.expires_at','Hết hạn lúc'))->default(fn()=>now()->addHours((int)config('dth-account-management.invitations.expires_hours',72)))->required()->seconds(false)->helperText(UiText::get('fields.expires_at_help','Sau thời điểm này, liên kết mời sẽ hết hiệu lực.')),
  Select::make('roles')->label(UiText::get('fields.roles','Vai trò'))->relationship('roles','name')->multiple()->searchable()->preload()->required()->helperText(UiText::get('fields.roles_help','Vai trò xác định các quyền mà người dùng sẽ được cấp.')),Select::make('groups')->label(UiText::get('fields.groups','Nhóm'))->relationship('groups','name')->multiple()->searchable()->preload()->helperText(UiText::get('fields.groups_help','Dùng để tổ chức người dùng theo phòng ban hoặc dự án.')),
 ...
 public static function table(Table $table):Table{return $table->columns([TextColumn::make('email')->label(UiText::get('common.fields.email','Email'))->searchable(),TextColumn::make('roles.name')->label(UiText::get('fields.roles','Vai trò'))->badge()->separator(','),TextColumn::make('status')->label(UiText::get('common.fields.status','Trạng thái'))->badge()->formatStateUsing(fn($state):string=>InvitationStatus::options()[$state instanceof \BackedEnum?$state->value:(string)$state]??(string)$state)->color(fn($state):string=>StatusColor::account($state)),TextColumn::make('expires_at')->label(UiText::get('fields.expires','Hết hạn'))->dateTime('d/m/Y H:i')->sortable(),TextColumn::make('invitedBy.name')->label(UiText::get('fields.invited_by','Người mời'))->placeholder('—')])->filters([SelectFilter::make('status')->options(InvitationStatus::options())])->recordActions([Actions\ActionGroup::make([
  ...
  ])->icon('heroicon-o-ellipsis-vertical')->iconButton()->visible(fn (): bool => app(AccountAuthorization::class)->allows('accounts.invitations.manage'))]);}
