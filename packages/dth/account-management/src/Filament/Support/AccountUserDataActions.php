...
use Dth\AccountManagement\Services\AccountUserDataExchangeService;
...
final class AccountUserDataActions
{
    public static function import(): Action
    {
        return Action::make('import_users')
            ->label(UiText::get('data.import', 'Nhập dữ liệu từ CSV/Excel'))
            ->icon('heroicon-o-arrow-up-tray')
            ->color('gray')
            ->extraAttributes(['class' => 'dth-acc-entry-action'])
            ->modalHeading(UiText::get('data.import_title', 'Nhập dữ liệu người dùng'))
            ->modalDescription(UiText::get('data.import_description', 'Tải lên tệp CSV hoặc XLSX. Cột bắt buộc: name, email.'))
            ...
            ->schema([
                FileUpload::make('file')
                    ...
                    ->hintIcon('heroicon-o-information-circle', tooltip: strtr(
                        ...
                        [':columns' => implode(', ', app(AccountUserDataExchangeService::class)->headers())],
                    )),
            ])
            ->action(function (array $data): void {
                try {
                    ...
                    $result = app(AccountUserDataExchangeService::class)->import($file->getRealPath(), $file->getClientOriginalExtension());

                    Notification::make()
                        ->title(UiText::get('data.import_success', 'Nhập dữ liệu hoàn tất'))
                        ->body(strtr(
                            UiText::get('data.import_summary', 'Tạo mới: :imported · Cập nhật: :updated · Lỗi: :errors'),
                            [':imported' => $result['imported'], ':updated' => $result['updated'], ':errors' => count($result['errors'])],
                        ))
                        ->success()
                        ->send();
                    ...
                    Notification::make()
                        ->title(UiText::get('data.import_failed', 'Nhập dữ liệu thất bại'))
