...
final class HtmlFormParser
{
    ...
    public function prepareForm(string $sourceHtml, FormAudienceType $audienceType): array
    {
        ...
        $visualBody = $this->buildVisualBody($safeSource, $formHtml, $fields);
        ...
        return [
            ...
            'schema' => [
                ...
                'presentation_assets' => $this->extractPresentationAssets($safeSource),
                'body_attributes' => $this->extractBodyAttributes($safeSource),
            ],
        ];
    }
    ...
    private function extractFields(string $formHtml, FormAudienceType $audienceType): array
    {
        ...
        foreach ($matches[0] as [$controlHtml, $offset]) {
            ...
            $fields[] = [
                'label' => $label !== '' ? $label : Str::headline($fieldKey),
                'field_key' => $fieldKey,
                ...
                'contact_mapping' => $this->suggestContactMapping($fieldKey, $fieldType, $audienceType),
    ...
    private function suggestContactMapping(
        string $fieldKey,
        FormFieldType $fieldType,
        FormAudienceType $audienceType,
    ): ?string {
        $key = Str::lower($fieldKey);

        $nameKeys = ['name', 'full_name', 'fullname', 'contact_name', 'customer_name', 'first_name', 'last_name', 'ho_ten', 'hoten', 'ten_khach_hang'];
        $emailKeys = ['email', 'email_address', 'contact_email', 'business_email', 'personal_email'];
        $phoneKeys = ['phone', 'phone_number', 'mobile', 'mobile_phone', 'telephone', 'tel', 'business_phone', 'dien_thoai', 'so_dien_thoai', 'sdt'];
        $serviceKeys = ['service', 'service_interest', 'interest', 'product', 'product_interest', 'package', 'service_package', 'dich_vu', 'san_pham', 'goi_dich_vu'];

        if (in_array($key, $nameKeys, true)) {
            return 'lead.name';
        }

        if (in_array($key, $emailKeys, true) || $fieldType === FormFieldType::Email) {
            return 'lead.email';
        }

        if (in_array($key, $phoneKeys, true) || $fieldType === FormFieldType::Phone) {
            return 'lead.phone';
        }
        ...
        if ($audienceType === FormAudienceType::Business) {
            if (in_array($key, ['company', 'company_name', 'organization', 'organisation', 'business_name', 'cong_ty', 'ten_cong_ty', 'doanh_nghiep', 'ten_doanh_nghiep'], true)) {
                return 'lead.company_name';
            }

            if (in_array($key, ['tax_code', 'tax_id', 'vat', 'mst', 'ma_so_thue'], true)) {
                return 'lead.tax_code';
            }

            if (in_array($key, ['position', 'job_title', 'contact_position', 'role', 'chuc_vu'], true)) {
