...
class CampaignReportPage extends Page implements HasForms
{
    ...
    public function exportCsv(): ?StreamedResponse
    {
        ...
        $filename = 'email-campaign-report-'.$campaign->id.'-'.now()->format('Ymd-His').'.csv';

        return response()->streamDownload(function () use ($campaign): void {
            $out = fopen('php://output', 'w');
            fwrite($out, "\xEF\xBB\xBF");
            fputcsv($out, [
                __('report.contact'), __('report.email'), __('report.status'), __('report.sent_at'),
                __('report.opened_at'), __('report.clicked_at'), __('report.failure_reason'),
            ]);
            ...
            foreach ($campaign->recipients as $recipient) {
                fputcsv($out, [
                    ...
                    $recipient->email,
                    CampaignRecipientStatus::tryFrom((string) $recipient->status)?->label() ?? $recipient->status,
                    optional($recipient->sent_at)->format('d/m/Y H:i:s'),
                    optional($recipient->opened_at)->format('d/m/Y H:i:s'),
                    optional($recipient->clicked_at)->format('d/m/Y H:i:s'),
    ...
    public function getStatsProperty(): array
    {
        ...
        $payments = Payment::query()->verified()
            ->whereHas('attribution', fn ($q) => $q->where('email_campaign_id', $campaign->id));
        $paidCustomers = (clone $payments)->whereNotNull('customer_id')->distinct('customer_id')->count('customer_id');
        $netRevenue = (float) (clone $payments)->sum('net_amount');
        ...
        return [
            ...
            'conversion_rate' => $sent > 0 ? round(($paidCustomers / $sent) * 100, 2) : 0,
        ];
    }
}
