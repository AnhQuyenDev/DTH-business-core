...
final class DashboardScopeService
{
    ...
    public function payments(User $user): Builder
    {
        ...
        if ($this->isCrossBusiness($user) || $user->isFinanceStaff() || $user->isSalesManager()) {
            return $query;
        }

        if ($user->isMarketingManager()) {
            return $query->whereHas('attribution', function (Builder $q): void {
                $q->whereNotNull('marketing_campaign_id')
                    ->orWhereNotNull('email_campaign_id')
                    ->orWhereNotNull('landing_page_id')
                    ->orWhereNotNull('utm_source')
                    ->orWhereNotNull('acquisition_source');
            });
        }
        ...
        if ($user->isMarketingStaff()) {
            return $query->whereHas('attribution', function (Builder $q) use ($user): void {
                $q->whereHas('landingPage', fn (Builder $sub): Builder => $sub->where('created_by', $user->id))
                    ->orWhereHas('marketingCampaign', fn (Builder $sub): Builder => $sub->where('created_by', $user->id))
                    ->orWhereHas('emailCampaign', fn (Builder $sub): Builder => $sub->where('created_by', $user->id));
            });
        }
