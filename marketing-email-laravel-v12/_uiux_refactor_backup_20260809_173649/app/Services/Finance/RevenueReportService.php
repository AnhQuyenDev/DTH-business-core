...
final class RevenueReportService
{
    ...
    {
        $base = $this->paymentQuery($filters);
        $financial = $this->financialRows($filters);
        ...
        $outstanding = (float) Quotation::query()
    ...
    /** @param array<string, mixed> $filters */
    public function marketingCampaigns(array $filters): array
    {
        [$start, $end] = $this->dateRange($filters);

        $rows = DB::query()
            ->fromSub($this->financialRows($filters), 'f')
        ...
        return $rows->map(function ($row) use ($leadCounts): array {
            ...
            return [
                'name' => $row->name ?: 'Chiến dịch #'.$row->id,
                'budget' => $budget,
                'leads' => $leads,
    ...
    /** @param array<string, mixed> $filters */
    public function emailCampaigns(array $filters): array
    {
        return DB::query()
            ->fromSub($this->financialRows($filters), 'f')
            ->join('payment_attributions', 'payment_attributions.payment_id', '=', 'f.payment_id')
    ...
    /** @param array<string, mixed> $filters */
    public function utmCampaigns(array $filters): array
    {
        return DB::query()
            ->fromSub($this->financialRows($filters), 'f')
            ->join('payment_attributions', 'payment_attributions.payment_id', '=', 'f.payment_id')
    ...
    /** @param array<string, mixed> $filters */
    public function landingPages(array $filters): array
    {
        [$start, $end] = $this->dateRange($filters);
        ...
        return $rows->map(function ($row) use ($leadCounts): array {
            ...
            return [
                'name' => $row->name ?: 'Landing Page #'.$row->id,
                'leads' => $leads,
                'payments' => (int) $row->payments_count,
                'paid_customers' => $paid,
                'net_revenue' => (float) $row->net_revenue,
    ...
    {
        return $this->lineBreakdown($filters, false);
    }

    /** @param array<string, mixed> $filters */
    public function packages(array $filters): array
    {
        return $this->lineBreakdown($filters, true);
    }

    /** @param array<string, mixed> $filters */
    private function lineBreakdown(array $filters, bool $package): array
    {
        $base = $this->paymentQuery($filters);
    ...
    {
        return DB::query()
            ->fromSub($this->financialRows($filters), 'p')
            ->leftJoin('staff', 'staff.id', '=', 'p.sales_staff_id')
            ->groupBy('p.sales_staff_id', 'staff.full_name')
            ->selectRaw("COALESCE(staff.full_name, 'Không xác định') as name, COUNT(DISTINCT p.payment_id) as payments_count, COUNT(DISTINCT p.customer_id) as paid_customers, SUM(p.net_amount) as net_revenue, SUM(p.gross_amount) as gross_collected")
            ->orderByDesc('net_revenue')
            ->limit(50)
            ->get()
            ->map(fn ($row): array => [
     ...
     * @param array<string, mixed> $filters
     */
    private function financialRows(array $filters)
    {
        $base = $this->paymentQuery($filters);
