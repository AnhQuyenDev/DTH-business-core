...
class LeadResource extends Resource
{
    ...
    public static function infolist(Infolist $infolist): Infolist
    {
        return $infolist
            ->schema([
                Section::make(__('section.lead_overview'))
                    ->description(__('helper.lead_overview'))
                    ->columns(['default' => 1, 'md' => 2, 'xl' => 3])
                    ->schema([
                        TextEntry::make('lead_code')
                        ...
                        TextEntry::make('source')
                            ...
                            ->formatStateUsing(fn ($state): string => Lead::sourceLabel(filled($state) ? (string) $state : null)),
                        TextEntry::make('source_detail')
                            ->label(__('field.source_detail'))
                            ->placeholder('—'),
                        ...
                        TextEntry::make('service_interest')
                            ...
                            ->formatStateUsing(fn ($state, Lead $record): string => (string) (
                                data_get($record->metadata, 'service_interest_label') ?? $state ?? '—'
                            )),
                ...
                Section::make(__('section.lead_processing'))
                    ->description(__('helper.lead_processing'))
                    ->columns(['default' => 1, 'md' => 2, 'xl' => 3])
                    ->schema([
                        ...
                        TextEntry::make('qualification.status')
                            ->label(__('field.processing_status'))
                            ->badge()
                        ...
                        TextEntry::make('qualification.qualification_result')
                            ...
                            ->badge()
                            ->formatStateUsing(fn ($state): string => $state?->label() ?? __('action.not_applicable')),
    ...
    {
        return $form->schema([
            FormSection::make(__('resource.lead.singular'))
                ->schema([
                    ...
                    Select::make('contact_id')->label(__('field.contact'))->relationship('contact', 'full_name')->searchable()->preload()->required(),
            ...
            FormSection::make(__('uiux.form.timeline'))
                ->schema([
                    Select::make('intake_status')->label(__('field.intake_status'))->options(LeadIntakeStatus::options())->native(false)->required(),
