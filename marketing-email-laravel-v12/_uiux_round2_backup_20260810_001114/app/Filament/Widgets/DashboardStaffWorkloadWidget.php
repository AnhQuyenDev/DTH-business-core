...
class DashboardStaffWorkloadWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        return [
            Stat::make(__(__('dashboard.staff.working')), number_format($active))
                ->description(__(__('dashboard.staff.present')).': '.number_format($available).' / '.__(__('dashboard.staff.on_leave')).': '.number_format($onLeave))
                ->icon('heroicon-o-users')
                ->color('success'),
            Stat::make(__(__('dashboard.staff.inactive').' / '.__('dashboard.staff.resigned')), number_format($inactive).' / '.number_format($resigned))
                ->icon('heroicon-o-pause-circle')
                ->color('warning'),
            Stat::make(__(__('dashboard.staff.managed_customers')), number_format($totalManaging))
                ->description(__(__('dashboard.staff.average')).': '.number_format($avgManaging).' '.__('khách hàng/nhân viên'))
                ->icon('heroicon-o-user-group')
                ->color('primary'),
            Stat::make(__(__('dashboard.staff.supported_customers')), number_format($totalSupporting))
                ->description(__(__('dashboard.staff.average')).': '.number_format($avgSupporting).' '.__('khách hàng/nhân viên'))
