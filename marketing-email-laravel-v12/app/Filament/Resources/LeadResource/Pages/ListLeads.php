        ...
        return [
            ...
            'not_contacted' => Tab::make(__('enum.qualification.not_contacted'))
                ->modifyQueryUsing(fn ($query) => $query->whereHas(
                    'qualification',
                    fn ($query) => $query->where('status', ContactQualificationStatus::New->value)
                )),
            ...
            'qualified' => Tab::make(__('enum.qualification.qualified'))
                ->modifyQueryUsing(fn ($query) => $query->whereHas(
                    'qualification',
                    fn ($query) => $query->where('status', ContactQualificationStatus::Qualified->value)
                )),
            ...
            'converted' => Tab::make(__('enum.qualification.converted'))
                ->modifyQueryUsing(fn ($query) => $query->whereHas(
                    'qualification',
                    fn ($query) => $query->where('status', ContactQualificationStatus::Converted->value)
                )),
        ];
    }
}
