        ...
        return $form->schema([
            Section::make(__('section.bank_account_details'))->schema([
                ...
                TextInput::make('account_number')
                    ...
                    ->regex('/^\d{6,19}$/')
                    ->maxLength(19)
                    ->helperText(__('helper.bank_account_number'))
                    ->suffixAction(
                        Action::make('lookup_account_name')
                            ->label(__('action.lookup'))
                            ...
                            ->action(function (Get $get, Set $set): void {
                                ...
                                if (blank($bankCode) || blank($accountNumber)) {
                                    Notification::make()
                                        ->warning()
                                        ->title(__('notification.bank_lookup_required'))
                                        ->send();
                                ...
                                Notification::make()
                                    ->success()
                                    ->title(__('notification.bank_account_verified'))
                                    ->body($name)
                                    ->send();
                            })
                    ),
                TextInput::make('account_name')
                    ...
                    ->required()
                    ->maxLength(255)
                    ->helperText(__('helper.bank_verified_name')),
                Select::make('status')
