    ...
    protected function getHeaderActions(): array
    {
        ...
        return array_filter([
            ...
            Action::make('send')->label(__('action.send'))
                ->form(function () use ($q): array {
                    ...
                    return [
                        ...
                        Select::make('authorized_signer_email')
                            ->label(__('field.authorized_signer'))
                            ->options($signers)
                            ->default($partyEmail)
                            ->required()
                            ->searchable()
                            ->helperText(__('helper.authorized_signer_otp')),
                ...
                ->action(function (array $data) use ($q): void {
                    ...
                    if ($log->status === QuotationEmailStatus::Failed) {
                        Notification::make()
                            ->danger()
                            ->title(__('notification.quotation_send_failed'))
                            ->body($log->error_message ?: __('notification.quotation_mail_failed'))
                            ->persistent()
                            ->send();
                    ...
                    Notification::make()
                        ...
                        ->title(
                            $log->status === QuotationEmailStatus::Sent
                                ? __('notification.quotation_sent')
                                : __('notification.quotation_queued')
                        )
                        ->body(__('field.from').': '.($log->sender_email ?: '—').' → '.$log->recipient_email)
                        ->send();
                })
            ...
            Action::make('create_revision')
                ->label(__('action.create_revision'))
                ->icon('heroicon-o-document-duplicate')
                ->color('warning')
                ->requiresConfirmation()
                ->modalHeading(__('action.create_quotation_revision'))
                ->modalDescription(__('helper.create_quotation_revision'))
