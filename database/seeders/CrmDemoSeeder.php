...
class CrmDemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $agentProfileIds = collect([
            ['employee_code' => 'EMP-DEMO-01', 'assignment_enabled' => true, 'lead_capacity' => 80, 'customer_capacity' => 120, 'distribution_weight' => 1.2],
            ['employee_code' => 'EMP-DEMO-02', 'assignment_enabled' => true, 'lead_capacity' => 60, 'customer_capacity' => 100, 'distribution_weight' => 1.0],
            ['employee_code' => 'EMP-DEMO-03', 'assignment_enabled' => true, 'lead_capacity' => 70, 'customer_capacity' => 100, 'distribution_weight' => 0.8],
            ['employee_code' => 'EMP-DEMO-04', 'assignment_enabled' => false, 'lead_capacity' => 40, 'customer_capacity' => 60, 'distribution_weight' => 0.5],
            ['employee_code' => 'EMP-DEMO-05', 'assignment_enabled' => true, 'lead_capacity' => 50, 'customer_capacity' => 80, 'distribution_weight' => 1.5],
            ...
            if (! $employeeId) {
                throw new \RuntimeException('Missing HR demo employee '.$row['employee_code'].'. Run HumanResourceDemoSeeder first.');
            }

            return DB::table('crm_agent_profiles')->insertGetId([
                'employee_id' => $employeeId,
                'assignment_enabled' => $row['assignment_enabled'],
                'lead_capacity' => $row['lead_capacity'],
                'customer_capacity' => $row['customer_capacity'],
        ...
        $companyIds = collect([
            ['company_code' => 'CMP-DEMO-01', 'legal_name' => 'VietStar Technology', 'tax_code' => '0101234501', 'email_domain' => 'vietstar.example', 'website' => 'https://vietstar.example', 'phone' => '0243000001', 'industry' => 'Technology', 'address' => '12 Cau Giay, Ha Noi', 'province' => 'Ha Noi', 'lifecycle_stage' => 'customer', 'account_owner_agent_profile_id' => $agentProfileIds[0]],
            ['company_code' => 'CMP-DEMO-02', 'legal_name' => 'GreenHub Services', 'tax_code' => '0101234502', 'email_domain' => 'greenhub.example', 'website' => 'https://greenhub.example', 'phone' => '0283000002', 'industry' => 'Services', 'address' => '88 Nguyen Hue, Ho Chi Minh', 'province' => 'Ho Chi Minh', 'lifecycle_stage' => 'qualified', 'account_owner_agent_profile_id' => $agentProfileIds[1]],
            ['company_code' => 'CMP-DEMO-03', 'legal_name' => 'Mekong Retail', 'tax_code' => '0101234503', 'email_domain' => 'mekong.example', 'website' => 'https://mekong.example', 'phone' => '0292000003', 'industry' => 'Retail', 'address' => '20 Tran Hung Dao, Can Tho', 'province' => 'Can Tho', 'lifecycle_stage' => 'prospect', 'account_owner_agent_profile_id' => $agentProfileIds[2]],
            ['company_code' => 'CMP-DEMO-04', 'legal_name' => 'Lotus Manufacturing', 'tax_code' => '0101234504', 'email_domain' => 'lotus.example', 'website' => null, 'phone' => '0236000004', 'industry' => 'Manufacturing', 'address' => '4 Le Loi, Da Nang', 'province' => 'Da Nang', 'lifecycle_stage' => 'inactive', 'account_owner_agent_profile_id' => null],
            ['company_code' => 'CMP-DEMO-05', 'legal_name' => 'Nova Education', 'tax_code' => '0101234505', 'email_domain' => 'nova.example', 'website' => 'https://nova.example', 'phone' => '0243000005', 'industry' => 'Education', 'address' => '5 Hoang Dieu, Ha Noi', 'province' => 'Ha Noi', 'lifecycle_stage' => 'prospect', 'account_owner_agent_profile_id' => $agentProfileIds[4]],
        ])->map(function (array $row) use ($now): int {
        ...
        foreach (range(0, 9) as $index) {
            ...
            $qualificationId = DB::table('crm_contact_qualifications')->insertGetId(['lead_id' => $leadId, 'contact_id' => $contactIds[$index % $contactIds->count()], 'assigned_agent_profile_id' => $agentProfileIds[$index % $agentProfileIds->count()], 'status' => $qualificationStatuses[$index], 'priority' => ['low', 'normal', 'high', 'urgent'][$index % 4], 'score' => 20 + ($index * 8), 'qualification_result' => in_array($qualificationStatuses[$index], ['qualified', 'converted'], true) ? 'qualified' : null, 'service_interest' => ['crm', 'business-pro', 'email', 'marketing-automation'][$index % 4], 'estimated_value' => 30000000 + ($index * 7000000), 'budget_status' => ['confirmed', 'estimated', 'unknown'][$index % 3], 'budget_amount' => 20000000 + ($index * 5000000), 'purchase_timeline' => ['immediate', 'within_1_month', 'within_3_months', 'within_6_months', 'later'][$index % 5], 'decision_role' => ['decision_maker', 'influencer', 'technical_contact'][$index % 3], 'qualification_note' => 'Qualification note for demo scenario '.$index, 'first_contacted_at' => $now->copy()->subDays($index), 'last_contacted_at' => $now->copy()->subDays(max(0, $index - 1)), 'next_follow_up_at' => $now->copy()->addDays($index + 1), 'qualified_at' => in_array($qualificationStatuses[$index], ['qualified', 'converted'], true) ? $now->copy()->subDays(2) : null, 'unqualified_reason' => $qualificationStatuses[$index] === 'unqualified' ? 'Budget is not approved yet.' : null, 'created_at' => $now, 'updated_at' => $now]);
            DB::table('crm_qualification_notes')->insert(['qualification_id' => $qualificationId, 'agent_profile_id' => $agentProfileIds[$index % $agentProfileIds->count()], 'note' => 'Follow-up note for qualification scenario '.$index, 'created_at' => $now, 'updated_at' => $now]);
        }

        $activityTypes = ['call', 'email', 'meeting', 'message', 'note', 'other'];
        foreach ($leadIds as $index => $leadId) {
            foreach (range(1, 3) as $activity) {
                DB::table('crm_lead_activities')->insert(['lead_id' => $leadId, 'agent_profile_id' => $agentProfileIds[($index + $activity) % $agentProfileIds->count()], 'type' => $activityTypes[($index + $activity) % count($activityTypes)], 'status' => $activity === 3 ? 'pending' : 'completed', 'subject' => 'Lead activity '.$activity, 'content' => 'Demo activity history for lead '.$leadId, 'outcome' => $activity === 1 ? 'follow_up' : null, 'activity_at' => $now->copy()->subDays($index + $activity), 'next_follow_up_at' => $activity === 3 ? $now->copy()->addDays(2) : null, 'metadata' => json_encode(['demo' => true]), 'created_at' => $now, 'updated_at' => $now]);
            }
        }
        ...
        foreach (range(1, 8) as $index) {
            ...
            $customerIds->push(DB::table('crm_customers')->insertGetId(['customer_code' => 'CUS-DEMO-'.str_pad((string) $index, 2, '0', STR_PAD_LEFT), 'contact_id' => $contactId, 'company_id' => $companyId, 'origin_lead_id' => $leadIds[$index - 1], 'customer_type' => $companyId ? 'business' : 'personal', 'display_name' => $companyId ? 'Business Customer '.$index : 'Personal Customer '.$index, 'email' => DB::table('crm_contacts')->where('id', $contactId)->value('email'), 'normalized_email' => DB::table('crm_contacts')->where('id', $contactId)->value('normalized_email'), 'phone' => DB::table('crm_contacts')->where('id', $contactId)->value('phone'), 'normalized_phone' => DB::table('crm_contacts')->where('id', $contactId)->value('normalized_phone'), 'acquisition_source' => ['landing_page', 'email', 'referral', 'direct'][$index % 4], 'consent_status' => $index % 3 === 0 ? 'pending' : 'approved', 'status' => ['potential', 'active', 'inactive', 'churned'][$index % 4], 'lifecycle_stage' => ['new_customer', 'repeat', 'loyal', 'at_risk'][$index % 4], 'conversion_reason' => $index % 2 === 0 ? 'purchased' : 'qualified_lead', 'converted_at' => $now->copy()->subDays($index), 'converted_by_agent_profile_id' => $agentProfileIds[$index % $agentProfileIds->count()], 'first_purchase_at' => $index % 2 === 0 ? $now->copy()->subDays($index + 2) : null, 'latest_purchase_at' => $index % 2 === 0 ? $now->copy()->subDays(1) : null, 'total_revenue' => $index % 2 === 0 ? 25000000 * $index : 0, 'priority' => ['low', 'normal', 'high', 'urgent'][$index % 4], 'next_follow_up_at' => $now->copy()->addDays($index), 'metadata' => json_encode(['demo' => true]), 'created_at' => $now, 'updated_at' => $now]));
        }
        foreach ($customerIds as $index => $customerId) {
            DB::table('crm_customer_assignments')->insert(['customer_id' => $customerId, 'agent_profile_id' => $agentProfileIds[$index % $agentProfileIds->count()], 'assignment_type' => $index % 3 === 0 ? 'support' : 'owner', 'status' => $index % 4 === 0 ? 'inactive' : 'active', 'reason' => 'demo_assignment', 'assigned_by_user_id' => $adminId, 'starts_at' => $now->copy()->subDays(10), 'ends_at' => $index % 4 === 0 ? $now->copy()->subDay() : null, 'created_at' => $now, 'updated_at' => $now]);
            foreach (range(1, 2) as $interaction) {
                DB::table('crm_customer_interactions')->insert(['customer_id' => $customerId, 'agent_profile_id' => $agentProfileIds[$index % $agentProfileIds->count()], 'interaction_type' => ['call', 'email', 'meeting', 'message', 'note', 'support'][($index + $interaction) % 6], 'subject' => 'Customer care interaction '.$interaction, 'content' => 'Demo customer interaction history.', 'outcome' => $interaction === 1 ? 'follow_up' : 'completed', 'interaction_at' => $now->copy()->subDays($interaction), 'next_follow_up_at' => $interaction === 1 ? $now->copy()->addDays(3) : null, 'is_support_action' => $interaction === 2, 'status' => $interaction === 2 ? 'pending' : 'completed', 'created_at' => $now, 'updated_at' => $now]);
            }
        }
        ...
        foreach ($customerIds as $index => $customerId) {
            DB::table('crm_customer_distribution_items')->insert(['distribution_batch_id' => $batchId, 'customer_id' => $customerId, 'original_owner_agent_profile_id' => null, 'assigned_agent_profile_id' => $agentProfileIds[$index % $agentProfileIds->count()], 'assignment_type' => 'owner', 'result_status' => $index % 4 === 0 ? 'skipped' : 'assigned', 'reason' => $index % 4 === 0 ? 'CRM agent capacity reached' : 'Round robin demo assignment', 'created_at' => $now, 'updated_at' => $now]);
        }
        foreach ($leadIds->take(8) as $index => $leadId) {
            DB::table('crm_lead_distribution_events')->insert(['lead_id' => $leadId, 'from_agent_profile_id' => null, 'to_agent_profile_id' => $agentProfileIds[$index % $agentProfileIds->count()], 'strategy' => 'round_robin', 'event' => 'assigned', 'status' => $index % 3 === 0 ? 'pending' : 'completed', 'reason' => 'Demo lead distribution', 'actor_user_id' => $adminId, 'responded_at' => $index % 3 === 0 ? null : $now->copy()->subDay(), 'created_at' => $now, 'updated_at' => $now]);
        }
        foreach ($companyIds as $index => $companyId) {
            if ($index < 4) {
                DB::table('crm_company_assignments')->insert(['company_id' => $companyId, 'agent_profile_id' => $agentProfileIds[$index % $agentProfileIds->count()], 'assignment_type' => 'owner', 'status' => $index === 3 ? 'inactive' : 'active', 'reason' => 'Demo ownership', 'assigned_by_user_id' => $adminId, 'starts_at' => $now->copy()->subDays(30), 'ends_at' => $index === 3 ? $now->copy()->subDay() : null, 'created_at' => $now, 'updated_at' => $now]);
            }
        }
        ...
        DB::table('crm_audit_logs')->insert(['event' => 'demo.seed.completed', 'subject_type' => 'demo', 'subject_id' => 'crm', 'actor_user_id' => $adminId, 'changes' => json_encode(['records' => ['agent_profiles' => $agentProfileIds->count(), 'contacts' => $contactIds->count(), 'companies' => $companyIds->count(), 'leads' => $leadIds->count(), 'customers' => $customerIds->count()]]), 'context' => json_encode(['source' => 'CrmDemoSeeder']), 'created_at' => $now, 'updated_at' => $now]);
    }
}
